...
function runPythonScript({ scriptPath, args = [], input, maxBuffer = 10 * 1024 * 1024, errorLabel = "Python script" }) {
  ...
  if (result.status !== 0) throw new Error(result.stderr || `${errorLabel} failed`);

  try {
    return JSON.parse(result.stdout);
  } catch (error) {
    throw new Error(`Invalid ${errorLabel} output: ${error.message}\n${result.stdout}`);
  }
}

function runPythonCnnPredict(imageBase64) {
  return runPythonScript({
    scriptPath: resolve(ROOT, "predict_cnn.py"),
    args: ["--model-path", resolve(ROOT, "models", "cnn_cifar10.keras")],
    input: imageBase64,
    maxBuffer: 20 * 1024 * 1024,
    errorLabel: "CNN prediction",
  });
}

function runPythonNlpPredict(text) {
  return runPythonScript({
    scriptPath: resolve(ROOT, "nlp_predict.py"),
    args: ["--text", text],
    errorLabel: "NLP",
  });
}
...
function formatVentureContext(venture) {
  return [
    `Name: ${venture?.name || ""}`,
    `Sector: ${venture?.sector || ""}`,
    `Stage: ${venture?.stage || ""}`,
...
async function aiWithFallback({ prompt, schema, localFn, errorField }) {
  let result;
  let source = AI_PROVIDER || "local";
  try {
    result = (await callAiJson(prompt, schema)) || localFn();
  } catch (error) {
    result = localFn();
    if (errorField && Array.isArray(result[errorField])) {
      result[errorField].push(`AI call failed: ${error.message}`);
    } else if (errorField && typeof result[errorField] === "string") {
      result[errorField] = `${result[errorField]} API call failed: ${error.message}`;
    }
    source = "local";
  }
...
async function routeApi(request, response, url) {
  ...
  if (request.method === "POST" && pathname === "/api/validate") {
    ...
    const { result, source } = await aiWithFallback({
      prompt: validationPrompt(venture),
      schema: validationSchema(),
      localFn: () => localValidation(venture),
      errorField: "summary",
    });
  ...
  if (request.method === "POST" && pathname === "/api/faq") {
    ...
    const { result, source } = await aiWithFallback({
      prompt: faqPrompt(question),
      schema: faqSchema(),
      localFn: () => localFaq(question),
      errorField: "next_steps",
    });
  ...
  if (request.method === "POST" && pathname === "/api/suggestions") {
    ...
    const { result, source } = await aiWithFallback({
      prompt: suggestionPrompt(message, payload.venture),
      schema: suggestionSchema(),
      localFn: () => localSuggestion(message, payload.venture),
      errorField: "risks_to_watch",
    });
  ...
  if (request.method === "POST" && pathname === "/api/roadmap") {
    ...
    const { result, source } = await aiWithFallback({
      prompt: roadmapPrompt(payload.venture, score),
      schema: roadmapSchema(),
      localFn: () => localRoadmap(payload.venture, score),
      errorField: "funding_readiness",
    });
