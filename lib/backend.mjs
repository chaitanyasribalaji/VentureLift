...
function runPythonVenturePredict(imageBase64) {
  if (process.env.VERCEL) {
    throw new Error(
      "Venture image classification requires Python and TensorFlow, " +
      "which are not available in Vercel serverless functions. " +
      "Run the app locally for full AI features."
    );
  }
  const scriptPath = resolve(ROOT, "predict_venture.py");
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr || "Venture image classification failed");
  }
...
function runPythonNlpPredict(text) {
  if (process.env.VERCEL) {
    throw new Error("Python NLP not available on Vercel");
  }
    ...
    if (!DatabaseSync) {
      try {
        ...
        throw new Error(
          ...
          "Set SUPABASE_URL and SUPABASE_KEY environment variables for cloud deployments."
        );
      }
    }
...
async function routeApi(request, response, url) {
  ...
  if (request.method === "POST" && pathname === "/api/cnn-predict") {
    ...
    try {
      const prediction = runPythonVenturePredict(imageBase64);
      sendJson(response, 200, { prediction });
    } catch (error) {
      sendJson(response, 500, { error: error.message || "Venture classification failed." });
    }
    return true;
  }
