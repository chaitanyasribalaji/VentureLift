...
async function supabaseInsert(table, record) {
  ...
  try {
    ...
    if (error) {
      console.error(`Supabase insert failed for ${table}:`, error.message || error);
      return null;
    }
    return data;
  } catch (error) {
    console.error(`Supabase insert exception for ${table}:`, error.message || error);
    return null;
  }
}
...
async function seedInitialUsers() {
  ...
  if (founderId == null) {
    console.error("Failed to seed founder user; skipping venture seeding.");
    return;
  }
...
async function getUserById(id) {
  if (USE_SUPABASE) {
    ...
    if (error && error.code !== "PGRST116") {
      console.error("Supabase getUserById error", error.message || error);
      throw new Error("Unable to look up user");
    }
    return data || null;
  }
...
async function listMentors(query) {
  ...
  if (USE_SUPABASE) {
    ...
    if (error) {
      console.error("Supabase listMentors error", error.message || error);
...
async function getVenturesForUser(user, query) {
  ...
  if (USE_SUPABASE) {
    ...
    if (error) {
      console.error("Supabase getVentures error", error.message || error);
      throw new Error("Unable to load ventures");
    }
    return data
      .filter((venture) => {
...
async function saveReport(ventureId, reportType, payload) {
  try {
    if (USE_SUPABASE) {
      const result = await supabaseInsert("ai_reports", {
        venture_id: ventureId || null,
        report_type: reportType,
        payload: JSON.stringify(payload),
        created_at: nowIso(),
      });
      if (!result) console.error(`Failed to persist ${reportType} report to Supabase`);
      return;
    }
    const db = getDb();
    ...
    console.error(`Failed to save ${reportType} report:`, error.message || error);
  }
}
