...
function runPythonMarketAnalysis(ventureName, sector, problem) {
  const scriptPath = resolve(ROOT, "market_trends.py");
  const pythonCommand = process.env.PYTHON || "python";
  const result = spawnSync(pythonCommand, [scriptPath, ventureName, sector, problem], {
    encoding: "utf8",
    maxBuffer: 10 * 1024 * 1024,
    timeout: 30000,
  });
  ...
  if (result.error) {
    return { error: "Market analysis unavailable", details: result.error.message };
  }
  if (result.status !== 0) {
    return { error: "Market analysis failed", details: result.stderr || "Unknown error" };
  }

  try {
    return JSON.parse(result.stdout);
  } catch (error) {
    return { error: "Invalid market data output", details: error.message };
  }
}
...
async function routeApi(request, response, url) {
  ...
  if (request.method === "POST" && pathname === "/api/login") {
    ...
    if (!email || !isEmailValid(email)) {
      sendJson(response, 400, { error: "Email is required and must be valid." });
      return true;
    }
    
    if (!password || password.length < 6) {
      sendJson(response, 400, { error: "Password is required and must be at least 6 characters." });
      return true;
    }
  ...
  if (request.method === "POST" && pathname === "/api/market-analysis") {
    const user = requireUser(request, response);
    if (!user) return true;
    const payload = await readJson(request);
    const venture = payload.venture || payload;
    
    if (!venture.name || !venture.sector || !venture.problem) {
      sendJson(response, 400, { error: "Venture name, sector, and problem are required." });
      return true;
    }

    const marketData = runPythonMarketAnalysis(venture.name, venture.sector, venture.problem);
    ...
    if (marketData.error) {
      sendJson(response, 200, { 
        result: {
          ...
          market_score: 50,
          market_fit: "unknown",
          insights: ["Market data temporarily unavailable - using default assessment"],
        },
        source: "local",
      });
      return true;
    ...
    await saveReport(venture.id, "market_analysis", marketData);
    sendJson(response, 200, { result: marketData, source: "market-api" });
    return true;
  }
