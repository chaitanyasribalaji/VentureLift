...
if (!process.env.JWT_SECRET) {
  console.warn("WARNING: JWT_SECRET is not set. Using a random per-process secret. Sessions will not survive restarts. Set JWT_SECRET in production.");
}
...
function setSessionCookie(response, token, expiresAt, request) {
  ...
  response.setHeader(
    "Set-Cookie",
    `vl_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Lax; Path=/; Expires=${new Date(expiresAt).toUTCString()}${secure}`,
  );
}

function clearSessionCookie(response, request) {
  const secure = isSecureContext(request) ? "; Secure" : "";
  response.setHeader("Set-Cookie", `vl_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${secure}`);
}
...
async function getLastValidationScore(ventureId) {
  ...
  if (USE_SUPABASE) {
    ...
    try {
      ...
      return Number(parsed.score) || null;
    } catch { return null; }
  }
  const db = getDb();
  const row = db.prepare("SELECT payload FROM ai_reports WHERE venture_id = ? AND report_type = 'validation' ORDER BY id DESC LIMIT 1").get(ventureId);
  if (!row) return null;
  try {
    return Number(JSON.parse(row.payload).score) || null;
  } catch { return null; }
}
...
function setSessionFromUser(response, user, request) {
...
async function routeApi(request, response, url) {
  ...
  if (request.method === "POST" && pathname === "/api/login") {
    if (!rateLimit(request, response)) return true;
    ...
    setSessionFromUser(response, user, request);
  ...
  if (request.method === "GET" && pathname === "/api/mentors") {
    const user = await requireRole(request, response, ["founder", "admin"]);
    if (!user) return true;
  ...
  if (request.method === "GET" && pathname === "/api/reports") {
    const user = await requireRole(request, response, ["admin", "mentor"]);
    if (!user) return true;
    if (USE_SUPABASE) {
  ...
  if (request.method === "POST" && pathname === "/api/validate") {
    ...
    try {
      ...
      console.warn("AI validation call failed:", error.message);
  ...
  if (request.method === "GET" && pathname === "/api/ai-status") {
    ...
    sendJson(response, 200, {
      ...
      supabase_enabled: user ? USE_SUPABASE : undefined,
    });
    return true;
  }
    ...
    try {
      if (AI_PROVIDER) {
        ...
        try {
          ...
          console.warn("Local NLP model failed:", err.message);
          result = localNlp(text);
          source = "local";
        }
      }
    } catch (error) {
      console.warn("NLP API call failed:", error.message);
      result = localNlp(text);
      source = "local";
    }
  ...
  if (request.method === "POST" && pathname === "/api/cnn-predict") {
    ...
    try {
      ...
      console.warn("CNN prediction failed:", error.message);
      sendJson(response, 500, { error: "Prediction failed." });
    }
    return true;
  }
  ...
  if (request.method === "POST" && pathname === "/api/faq") {
    ...
    try {
      ...
      console.warn("FAQ AI call failed:", error.message);
  ...
  if (request.method === "POST" && pathname === "/api/suggestions") {
    ...
    try {
      ...
      console.warn("Suggestion AI call failed:", error.message);
  ...
  if (request.method === "POST" && pathname === "/api/roadmap") {
    ...
    if (!user) return true;
    const payload = await readJson(request);
    const ventureId = payload.venture?.id;
    const serverScore = await getLastValidationScore(ventureId);
    ...
    try {
      ...
      console.warn("Roadmap AI call failed:", error.message);
